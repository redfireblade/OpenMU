...
/// <summary>
/// 事件活动广播器 — 监听 MiniGameDefinition 的开放/关闭状态，定期广播提醒。
///
/// 工作原理：
///   1. 每 ~10 秒扫描一次所有 MiniGameDefinition
///   2. 通过 IPeriodicMiniGameStartPlugIn 推断当前 PeriodicTaskState
///   3. 状态迁移时发布事件到 AiEventBus
///
...
/// </summary>
public sealed class EventWatcherService
{
    private readonly AiPlayer _player;
    private readonly ILogger _logger;
    private readonly AiEventBus _eventBus;
    ...
    /// <param name="eventBus">The AI event bus to publish events to.</param>
    /// <param name="logger">The logger.</param>
    public EventWatcherService(AiPlayer player, AiEventBus eventBus, ILogger logger)
    {
        this._player = player;
    ...
    public async ValueTask CheckEventsAsync()
    {
        ...
        if (now - this._lastScanTime < ScanInterval)
        {
            return;
        }
        ...
        {
            // 等级过滤
            if (level < mg.MinimumCharacterLevel)
            {
                continue;
            }

            if (mg.MaximumCharacterLevel > 0 && level > mg.MaximumCharacterLevel)
            {
                continue;
            }
            ...
            if (!this._knownStates.TryGetValue(key, out var prevState))
            {
                ...
                {
                    // 启动时已开放 → 发布提醒以同步
                    this._eventBus.Publish(new EventReminderEvent(mg.Type, (int)mg.GameLevel, nameStr, -1));
                }
                else if (currentState == PeriodicTaskState.Prepared)
                {
                    // 启动时已在准备中 → 发布开放事件
                    this._eventBus.Publish(new EventOpenEvent(mg.Type, (int)mg.GameLevel, nameStr, mg.EntranceFee));
                }
            ...
            if (prevState != currentState)
            {
                ...
                {
                    // 正式开放（入场变为可进入）
                    this._eventBus.Publish(new EventReminderEvent(mg.Type, (int)mg.GameLevel, nameStr, 0));
                ...
                {
                    // 活动结束
                    this._eventBus.Publish(new EventClosedEvent(mg.Type, (int)mg.GameLevel, nameStr));

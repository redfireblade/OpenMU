        ...
        if (pendingTest is not null)
        {
            ...
            else if (testType == "rule_engine_chain")
            {
                ...
                if (chainResult is not null)
                {
                    ...
                    // 学习规则(AutoGenerated) → 提交 MissionBoard 任务看板
                    if (!handled && chainResult.Rule.AutoGenerated)
                    {
                        var mission = chainResult.GeneratedMission;
                        if (mission is not null)
                        {
                            this._missionBoard.SyncPlayerState(); // 刷新看板
                            this._context.RecordDecision(mission.Module, TimeSpan.Zero);
                            this._logger.LogInformation(
                                "[HB] LearnRule executed: {RuleId} → {Module} (confidence={Conf})",
                                chainResult.Rule.RuleId, mission.Module, chainResult.Rule.Confidence);
                        }
                    }
                    else if (!handled)
                    {
                        this._logger.LogDebug("[HB] 规则 {RuleId} 无特殊处理", chainResult.Rule.RuleId);
                    }

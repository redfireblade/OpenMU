        ...
        lock (_oapsLock)
        {
            ...
            try
            {
                ...
                _syncTimer = new Timer(_ =>
                {
                    ...
                    // 将共享 RuleEngine 中的学习规则同步到每个 AI 角色的 HeartbeatService
                    if (_sharedRuleEngine?.Rules.Any(r => r.AutoGenerated) == true)
                    {
                        foreach (var kvp in _activePlayers)
                        {
                            try { kvp.Value.Logic?.GetHeartbeat()?.SyncLearnedRules(_sharedRuleEngine); }
                            catch { }
                        }
                    }

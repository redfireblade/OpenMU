...
{
    /// <summary>
    /// Tests that goto @label switches to the target paragraph.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Tests that goto to a non-existent label does not throw.
    /// </summary>
    [Fact]
    public async Task GotoLabel_NonExistent_DoesNotThrow()
    {
        ...
        var exception = await Record.ExceptionAsync(() => executor.TickAsync().AsTask());
    ...
    /// <summary>
    /// Tests that a paragraph without goto loops on the current paragraph.
    /// </summary>
    [Fact]
    public async Task ParagraphWithoutGoto_LoopsCurrentParagraph()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (await executor.TickAsync()) tickCount++;
        }
    ...
    /// <summary>
    /// Tests that goto chaining through multiple paragraphs works.
    /// </summary>
    [Fact]
    public async Task MultipleParagraphs_GotoChaining()
    {
        ...
        Assert.Equal(1, script.BranchHitCounts.GetValueOrDefault("a_to_b.if"));
        Assert.Equal(1, script.BranchHitCounts.GetValueOrDefault("b_to_c.if"));
        Assert.Equal(1, script.BranchHitCounts.GetValueOrDefault("c_stay.if"));
    }

    /// <summary>
    /// Tests that paragraph with goto on elif switches correctly.
    /// </summary>
    [Fact]
